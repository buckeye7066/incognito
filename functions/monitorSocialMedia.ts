...
Deno.serve(async (req) => {
  try {
    ...
    // Self-test mode
    if (body._selfTest === '1') {
      return Response.json({ ok: true, testMode: true, function: 'monitorSocialMedia' });
    }
...
Only include findings with exact vault data matches. No assumptions or generic matches.`;
    ...
    console.error('Social media monitoring error occurred');
    return Response.json({ 
      error: 'Failed to monitor social media',
      details: 'An error occurred during monitoring'
    }, { status: 500 });
  }
});